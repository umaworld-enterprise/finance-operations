...
class DepositRequest(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    ...
    # Financial data
    currency: Mapped[CurrencyCode] = mapped_column(pg_enum(CurrencyCode, "currency_code"), nullable=False)
    ...
    deposit_percentage: Mapped[float] = mapped_column(Numeric(5, 2), nullable=False)
    total_supplier_invoice_amount: Mapped[float] = mapped_column(Numeric(18, 2), nullable=False)

    # Dates
    estimated_shipment_date: Mapped[date] = mapped_column(Date, nullable=False)
