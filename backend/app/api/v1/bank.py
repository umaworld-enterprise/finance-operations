"""Banking module endpoints (Aug 2026) — super-admin only for now.
...
async def list_statements(current_user: SuperAdmin, db: DB) -> list[BankStatementResponse]:
...
async def get_statement(
    statement_id: UUID, current_user: SuperAdmin, db: DB
) -> BankStatementDetailResponse:
    result = await db.execute(
